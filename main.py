...
def recepcionAjax():
    if request.method == 'PUT':
      ...
      if tipo == "zapatillas":
            conn = sqlite3.connect("baseCulture.db")
            q = f"""UPDATE Zapatillas
            SET descripcionZapatilla = '{descripcion}'
            WHERE nombreZapatilla = '{producto}'"""
            print(q)
            resu = conn.execute(q)
            producto = resu.fetchall()
            print(producto)
      ...
      if tipo == "pantalones":
            conn = sqlite3.connect("baseCulture.db")
            q = f"""UPDATE Pantalones 
            SET descripcionPantalon = '{descripcion}'
            WHERE nombrePantalon = '{producto}'"""
            print(q)
            resu = conn.execute(q)
            print(q)
            producto = resu.fetchall()
      ...
      if tipo == "remeras":
            conn = sqlite3.connect("baseCulture.db")
            q = f"""UPDATE Remeras
            SET descripcionRemera
            WHERE nombreRemera = '{producto}'"""
            resu = conn.execute(q)
            print(q)
            producto = resu.fetchall()
            print(producto)
